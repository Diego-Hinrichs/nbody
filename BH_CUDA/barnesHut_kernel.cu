...
__global__ void ComputeForceKernel(Node *node, Body *bodies, int nNodes, int nBodies, int leafLimit)
{
    ...
    if (i < nBodies)
    {
        ...
        if (bi.isDynamic)
        {
            ...
            // Computa la fuerza recursivamente
            // Print body before and after update
            // printf("Before update - Body %d: Position (%f, %f, %f), Velocity (%f, %f, %f), Acceleration (%f, %f, %f)\n",i, bi.position.x, bi.position.y, bi.position.z, bi.velocity.x, bi.velocity.y, bi.velocity.z, bi.acceleration.x, bi.acceleration.y, bi.acceleration.z);
            ...
            // Print body after update
            // printf("After update - Body %d: Position (%f, %f, %f), Velocity (%f, %f, %f), Acceleration (%f, %f, %f)\n",i, bi.position.x, bi.position.y, bi.position.z, bi.velocity.x, bi.velocity.y, bi.velocity.z, bi.acceleration.x, bi.acceleration.y, bi.acceleration.z);
        }
    }
}
